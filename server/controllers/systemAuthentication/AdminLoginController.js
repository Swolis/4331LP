...
var AdminLoginController = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, username, password, ListClientModel, user, isMatch, token, error_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            ...
            case 1:
                _b.trys.push([1, 4, , 5]);
                ListClientModel = mongoose_1.default.model('ListClient');
                return [4 /*yield*/, ListClientModel.findOne({ username: username })];
            case 2:
                user = _b.sent();
                if (!user) {
                    return [2 /*return*/, res.status(401).json({ message: 'Authentication Failed.' })];
                }
                return [4 /*yield*/, bcrypt.compare(password, user.hashedPassword)];
            case 3:
                isMatch = _b.sent();
                if (isMatch) {
                    token = jwt.sign({ username: username, databaseName: user.databaseName }, SecretKey, { expiresIn: '1h' });
                    res.cookie('token', token, { httpOnly: true, secure: true });
                    return [2 /*return*/, res.status(200).json({ message: 'Authentication Successful.' })];
                }
                else {
                    return [2 /*return*/, res.status(400).json({ message: 'Authentication Failed.' })];
                }
                return [3 /*break*/, 5];
            case 4:
                error_1 = _b.sent();
